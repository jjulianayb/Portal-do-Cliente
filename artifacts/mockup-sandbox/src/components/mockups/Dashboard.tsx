...
function EmployeeHistory({ employees, selectedEmployeeId, onSelectEmployee, checkins, feedbacks, pdis, assessments, cycles, disciplinaryActions, disciplinaryApprovals, areaNames, positionNames, cycleNames, policyNames }: { employees: Employee[]; selectedEmployeeId: string; onSelectEmployee: (id: string) => void; checkins: Checkin[]; feedbacks: Feedback[]; pdis: Pdi[]; assessments: Assessment[]; cycles: Cycle[]; disciplinaryActions: DisciplinaryAction[]; disciplinaryApprovals: DisciplinaryApproval[]; areaNames: Map<string, string>; positionNames: Map<string, string>; cycleNames: Map<string, string>; policyNames: Map<string, string> }) {
  ...
  const timeline = [
    ...employeeCheckins.map((item) => ({ id: `checkin-${item.id}`, date: item.checkin_date, label: "Check-in diário", title: `Clima ${item.mood}/5 · Engajamento ${item.engagement}/5`, description: item.note ?? "Leitura diária registrada pela liderança.", tone: "blue", meta: `Energia ${item.energy}/5 · Carga ${item.workload}/5` })),
    ...employeeFeedbacks.map((item) => ({ id: `feedback-${item.id}`, date: item.created_at.slice(0, 10), label: "Feedback", title: "Conversa de desenvolvimento", description: item.content, tone: "emerald", meta: item.visibility === "private" ? "Registro privado" : "Registro compartilhado" })),
    ...employeePdis.map((item) => ({ id: `pdi-${item.id}`, date: item.created_at?.slice(0, 10) ?? item.due_date ?? "", label: "PDI", title: item.objective, description: item.due_date ? `Prazo: ${formatDate(item.due_date)}` : "Sem prazo definido.", tone: "violet", meta: statusLabel(item.status) })),
    ...employeeAssessments.map((item) => ({ id: `assessment-${item.id}`, date: item.created_at.slice(0, 10), label: "Avaliação", title: cycleNames.get(item.cycle_id) ?? "Avaliação de desempenho", description: "Avaliação vinculada ao ciclo de desenvolvimento.", tone: "amber", meta: "Registro criado" })),
    ...employeeActions.map((item) => ({ id: `disciplinary-${item.id}`, date: item.applied_at, label: "Medida disciplinar", title: item.action_type, description: item.reason, tone: "red", meta: `${policyNames.get(item.policy_id ?? "") ? `${policyNames.get(item.policy_id ?? "")} · ` : ""}${approvalStatusLabel(item.approval_status)}` })),
