...
type DisciplinaryAction = { id: string; employee_id: string; policy_id?: string | null; action_type: string; reason: string; notes?: string | null; approval_status: "approved" | "pending_facilitator" | "pending_rh" | "rejected"; applied_at: string; created_at: string };
...
function approvalStatusLabel(status: string): string {
  return ({ approved: "Aprovada", pending_facilitator: "Aguardando facilitador", pending_rh: "Aguardando RH", rejected: "Rejeitada" } as Record<string, string>)[status] ?? status;
}
...
export default function Dashboard({ session, organization, onLogout }: DashboardProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [employeeRows, areaRows, positionRows, cycleRows, feedbackRows, pdiRows, assessmentRows, disciplinaryRows, approvalRows, policyRows, checkinRows] = await Promise.all([
        ...
        apiRequest<DisciplinaryApproval[]>(session, `disciplinary_action_approvals?select=id,action_id,approver_type,status,note,reviewed_at&organization_id=eq.${org}&order=created_at.desc`),
  ...
  async function createDisciplinaryAction(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const approvalStatus = selectedPolicy?.requires_facilitator_approval ? "pending_facilitator" : selectedPolicy?.requires_hr_approval ? "pending_rh" : "approved";
      ...
      if (action && selectedPolicy) {
        ...
        if (selectedPolicy.requires_facilitator_approval) approvals.push({ organization_id: organization.id, action_id: action.id, approver_type: "facilitador", status: "pending" });
