...
export default function Dashboard({ session, organization, onLogout }: DashboardProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [employeeRows, areaRows, positionRows, cycleRows, feedbackRows, pdiRows, assessmentRows, disciplinaryRows, approvalRows, approverRows, policyRows, checkinRows] = await Promise.all([
        ...
        apiRequest<OrganizationApprover[]>(session, `organization_approvers?select=id,approver_label,email,active&organization_id=eq.${org}&active=eq.true&order=approver_label,email`),
  ...
  async function createOrganizationApprover(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); resetMessages(); setSaving(true);
    try {
      await apiRequest(session, "organization_approvers", { method: "POST", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ organization_id: organization.id, approver_label: approverLabel.trim(), email: approverEmail.trim().toLowerCase(), active: true }) });
      setApproverEmail(""); setNotice("Responsável autorizado para aprovações intermediárias."); await loadData();
    } catch (saveError) { setError(saveError instanceof Error ? saveError.message : "Não foi possível cadastrar o responsável."); } finally { setSaving(false); }
  }
