...
export default function Dashboard({ session, organization, onLogout }: DashboardProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const approvalRows = await apiRequest<DisciplinaryApproval[]>(session, `disciplinary_action_approvals?select=id,action_id,approver_type,approver_label,status,note,reviewed_at&organization_id=eq.${org}&${approvalScope}&order=created_at.desc`);
