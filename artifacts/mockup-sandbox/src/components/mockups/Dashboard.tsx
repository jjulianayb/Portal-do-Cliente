...
type DisciplinaryPolicy = { id: string; name: string; description?: string | null; requires_facilitator_approval: boolean; requires_hr_approval: boolean; active: boolean; sequence_order: number };
type DisciplinaryAction = { id: string; employee_id: string; policy_id?: string | null; action_type: string; reason: string; notes?: string | null; applied_at: string; created_at: string };
...
export default function Dashboard({ session, organization, onLogout }: DashboardProps) {
  ...
  const [policyRequiresFacilitatorApproval, setPolicyRequiresFacilitatorApproval] = useState(false);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [employeeRows, areaRows, positionRows, cycleRows, feedbackRows, pdiRows, assessmentRows, disciplinaryRows, policyRows, checkinRows] = await Promise.all([
        ...
        apiRequest<DisciplinaryAction[]>(session, `disciplinary_actions?select=id,employee_id,policy_id,action_type,reason,notes,applied_at,created_at&organization_id=eq.${org}&order=applied_at.desc,created_at.desc`),
        apiRequest<DisciplinaryPolicy[]>(session, `disciplinary_policies?select=id,name,description,requires_facilitator_approval,requires_hr_approval,active,sequence_order&organization_id=eq.${org}&active=eq.true&order=sequence_order,name`),
  ...
  async function createDisciplinaryPolicy(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); resetMessages(); setSaving(true);
    try {
      await apiRequest(session, "disciplinary_policies", { method: "POST", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ organization_id: organization.id, name: policyName.trim(), description: policyDescription.trim() || null, requires_facilitator_approval: policyRequiresFacilitatorApproval, requires_hr_approval: policyRequiresApproval, sequence_order: disciplinaryPolicies.length + 1, active: true }) });
      setPolicyName(""); setPolicyDescription(""); setPolicyRequiresFacilitatorApproval(false); setPolicyRequiresApproval(false); setNotice("Regra disciplinar adicionada à política da empresa."); await loadData();
  ...
  async function createDisciplinaryAction(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); resetMessages(); setSaving(true);
    try {
      await apiRequest(session, "disciplinary_actions", { method: "POST", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ organization_id: organization.id, employee_id: disciplinaryEmployee, policy_id: disciplinaryPolicy || null, action_type: disciplinaryPolicy ? (disciplinaryPolicies.find((policy) => policy.id === disciplinaryPolicy)?.name ?? disciplinaryType) : disciplinaryType, reason: disciplinaryReason.trim(), notes: disciplinaryNotes.trim() || null, applied_at: disciplinaryDate }) });
      setDisciplinaryEmployee(""); setDisciplinaryPolicy(""); setDisciplinaryReason(""); setDisciplinaryNotes(""); setNotice("Medida disciplinar registrada no histórico do colaborador."); await loadData();
