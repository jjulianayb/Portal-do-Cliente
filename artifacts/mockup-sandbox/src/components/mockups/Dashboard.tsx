...
type DisciplinaryPolicy = { id: string; name: string; description?: string | null; requires_hr_approval: boolean; active: boolean; sequence_order: number };
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [employeeRows, areaRows, positionRows, cycleRows, feedbackRows, pdiRows, assessmentRows, disciplinaryRows, policyRows, checkinRows] = await Promise.all([
        ...
        apiRequest<DisciplinaryPolicy[]>(session, `disciplinary_policies?select=id,name,description,requires_hr_approval,active,sequence_order&organization_id=eq.${org}&active=eq.true&order=sequence_order,name`),
