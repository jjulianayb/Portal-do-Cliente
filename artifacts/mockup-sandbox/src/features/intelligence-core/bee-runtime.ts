...
export type BeeProvenance = { entityType: BeeEntityKind | "ASSESSMENT" | "MEMORY" | "EVENT"; entityId: string; relationship?: string; sourceEntityId?: string; };
...
export type BeeReadingBundle = { finding: BeeRuntimeFinding; hypotheses: readonly BeeRuntimeFinding[]; sources: readonly BeeProvenance[]; evidence: readonly BeeEvidenceNode[]; assessments: readonly BeeAssessmentNode[]; recommendations: readonly BeeRecommendationNode[]; decisions: readonly BeeDecisionNode[]; };
...
export function composeBeeReadModel(context: BeeRuntimeContext, source: BeeRuntimeSource, now = new Date()): BeeRuntimeReadModel {
  const evidenceById = new Map(source.evidence.filter((item) => item.organization_id === context.organizationId).map((item) => [item.id, item]));
  ...
  const recById = new Map(source.recommendations.filter((item) => allowedSource(context, item, item.scope_type ?? (item.employee_id ? "employee" : "organization"), item.scope_ref ?? item.employee_id ?? context.organizationId, item.employee_id)).map((item) => [item.id, item]));
  ...
  const recEvidence = source.recommendationEvidence.filter((item) => recById.has(item.recommendation_id) && evidenceById.has(item.evidence_id));
  const personalActionIds = new Set(source.actions.filter((item) => item.organization_id === context.organizationId && (!personalOnly(context) || item.assignee_employee_id === context.employeeId)).map((item) => item.id));
  const interventionById = new Map(source.interventions.filter((item) => item.organization_id === context.organizationId && (!personalOnly(context) || item.employee_id === context.employeeId)).map((item) => [item.id, item]));
  ...
  const decisions = source.decisions.filter((item) => item.organization_id === context.organizationId && (scopeAllowed(context, item.scope_type, item.scope_ref, item.owner_employee_id) || (personalOnly(context) && item.owner_employee_id === context.employeeId))).map(makeDecision);
  const actions = source.actions.filter((item) => item.organization_id === context.organizationId && (!personalOnly(context) || item.assignee_employee_id === context.employeeId) && (!personalOnly(context) || personalActionIds.has(item.id))).map((item) => ({ finding: finding("ACTION", item.id, item.organization_id, item.assignee_employee_id ? "employee" : null, item.assignee_employee_id ?? null, item.title, item.title, item.status, "declared", [], [], [{ entityType: "ACTION", entityId: item.id }], null, { createdAt: item.due_at, updatedAt: item.completed_at }, item.intervention_id ? [{ entityType: "INTERVENTION", entityId: item.intervention_id, relationship: "derived_from" }] : []), interventionId: item.intervention_id ?? null, dueAt: item.due_at ?? null, completedAt: item.completed_at ?? null }));
  const actionIds = new Set(actions.map((item) => item.finding.id));
  const outcomes = source.outcomes.filter((item) => item.organization_id === context.organizationId && (!personalOnly(context) || (item.action_id ? actionIds.has(item.action_id) : !!item.intervention_id && interventionById.has(item.intervention_id)))).map((item) => ({ finding: finding("OUTCOME", item.id, item.organization_id, null, null, item.outcome_type, item.details?.summary ? String(item.details.summary) : item.outcome_type, item.status, "observed", [], [], [{ entityType: "OUTCOME", entityId: item.id }], null, { observedAt: item.measured_at, createdAt: item.measured_at }), interventionId: item.intervention_id ?? null, actionId: item.action_id ?? null, validationStatus: item.validation_status ?? null }));
  const readings = [...readingById.values()].map((reading) => {
    ...
    const readingAssessments = assessments.filter((item) => item.finding.links.some((link) => link.entityId === reading.id) || assessmentById.get(item.finding.id)?.reading_id === reading.id);
    ...
    const sourceRefs = source.organizationalReadingSources.filter((item) => item.reading_id === reading.id).map((item) => ({ entityType: item.source_type === "evidence" ? "EVIDENCE" as const : "EVENT" as const, entityId: item.evidence_id ?? item.organizational_event_id ?? item.id, relationship: item.relationship_type }));
  ...
  const memory = source.organizationalMemoryRelations.filter((item) => item.organization_id === context.organizationId && sensitivityAllowed(context, item.sensitivity) && (item.source_entity_type === "reading" ? readingById.has(item.source_entity_id) : !personalOnly(context))).map((item) => ({ entityId: item.source_entity_id, sourceType: item.source_type, relationship: item.relationship_type, knowledgeKind: item.knowledge_kind, sensitivity: item.sensitivity, provenance: [{ entityType: item.source_entity_type === "recommendation" ? "RECOMMENDATION" : item.source_entity_type === "reading" ? "READING" : "MEMORY", entityId: item.source_entity_id, relationship: item.relationship_type }] }));
  const events = source.organizationalEvents.filter((item) => item.organization_id === context.organizationId && sensitivityAllowed(context, item.sensitivity) && !personalOnly(context)).map((item) => ({ id: item.id, eventType: item.event_type, entityType: item.entity_type, entityId: item.entity_id, occurredAt: item.occurred_at, sensitivity: item.sensitivity, provenance: [{ entityType: "EVENT" as const, entityId: item.id }] }));
  const model: BeeRuntimeReadModel = { context: { organizationId: context.organizationId, userId: context.userId, role: context.role, employeeId: context.employeeId, purpose: context.purpose }, readings, assessments, recommendations, decisions, actions, outcomes, memory, events, attentionToday: [] };
...
export function buildBeeAttentionToday(context: BeeRuntimeContext, model: BeeRuntimeReadModel, limit = 5, now = new Date()): BeeAttentionItem[] {
  ...
  for (const reading of model.readings) if ((reading.finding.status === "open" || reading.finding.status === "under_investigation") && reading.finding.title.toLowerCase().includes("risk")) items.push({ rule: "open_risk_reading", finding: reading.finding, explanation: "Leitura de risco aberta requer atenção." });
