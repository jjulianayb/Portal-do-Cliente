...
export function composeBeeReadModel(context: BeeRuntimeContext, source: BeeRuntimeSource, now = new Date()): BeeRuntimeReadModel {
  ...
  const memory = source.organizationalMemoryRelations.filter((item) => item.organization_id === context.organizationId && sensitivityAllowed(context, item.sensitivity) && (item.source_entity_type === "reading" ? readingById.has(item.source_entity_id) : !personalOnly(context))).map((item) => ({ entityId: item.source_entity_id, sourceType: item.source_type, relationship: item.relationship_type, knowledgeKind: item.knowledge_kind, sensitivity: item.sensitivity, provenance: [{ entityType: "MEMORY", entityId: item.source_entity_id, relationship: item.relationship_type }] }));
