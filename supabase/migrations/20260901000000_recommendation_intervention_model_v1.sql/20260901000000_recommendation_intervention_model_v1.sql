...
-- Additive structural foundation. No IA, scoring, ranking, matching or automatic execution.
...
alter table public.intelligence_recommendations
  ...
  add column if not exists recommended_intervention_type text,
  add column if not exists alternatives jsonb not null default '[]'::jsonb,
  add column if not exists do_not_recommend jsonb not null default '[]'::jsonb,
...
alter table public.intelligence_interventions
  add column if not exists intervention_family text,
  add column if not exists objective text,
  add column if not exists target_scope_type text,
  add column if not exists target_scope_ref text,
...
comment on table public.intelligence_recommendations is 'Structured, explainable Recommendation contract for Leitura Organizacional. No automatic generation, scoring or execution.';
comment on column public.intelligence_recommendations.scope_ref is 'Descriptive target reference only; never an authorization mechanism.';
...
comment on column public.intelligence_recommendations.problem_statement is 'Factual, non-diagnostic statement of what merits attention.';
comment on column public.intelligence_recommendations.evidence_state is 'Authorized future flow input only; V1 performs no evidence calculation.';
...
comment on column public.intelligence_interventions.target_scope_ref is 'Descriptive target reference only; never an authorization mechanism.'
