...
insert into ctx values
 ...
 ('decision_org_b','44444444-0000-0000-0000-000000000037'),('decision_memory','33333333-0000-0000-0000-000000000038'),
...
update public.intelligence_decisions set supersedes_decision_id=(select value from ctx where key='decision_a'), status='superseded' where id=(select value from ctx where key='decision_superseded');
insert into public.intelligence_decisions(
...
create or replace function pg_temp.directoria_decision_insert_ok() returns boolean language plpgsql security invoker as $$ declare decision_id uuid := '33333333-0000-0000-0000-000000000052'; begin
  insert into public.intelligence_decisions(id,organization_id,decision_type,scope_type,scope_ref,decision_statement,alternatives_considered,evidence_snapshot,unknowns,risk_level,status,decision_maker_user_id,approval_required,context)
  values(decision_id,(select value from ctx where key='org_a'),'maintain','organization','org-a','Diretoria decision','[]','{}','[]','operational','decided',(select value from ctx where key='user_2'),false,'{}'); return exists(select 1 from public.intelligence_decisions where id=decision_id);
end; $$;
...
update public.intelligence_decisions set approved_by=(select value from ctx where key='user_2'),approved_at='2026-02-02',status='decided' where id=(select value from ctx where key='decision_pending');
...
create or replace function pg_temp.rh_decision_insert_ok() returns boolean language plpgsql security invoker as $$ declare decision_id uuid := '33333333-0000-0000-0000-000000000053'; begin
  insert into public.intelligence_decisions(id,organization_id,decision_type,scope_type,scope_ref,decision_statement,alternatives_considered,evidence_snapshot,unknowns,risk_level,status,decision_maker_user_id,approval_required,context)
  values(decision_id,(select value from ctx where key='org_a'),'defer','employee',(select value from ctx where key='employee_a'),'RH deferred decision','[]','{"available":[]}','["more evidence"]','personal_reversible','pending_review',(select value from ctx where key='user_5'),false,'{}'); return exists(select 1 from public.intelligence_decisions where id=decision_id);
end; $$;
